...
fn render_dual_pane(
    ...
    let feed_items: Vec<ListItem> = if feeds.is_empty() {
        ...
        feeds
            .iter()
            .map(|feed| {
                // Query feeds are shown with a special indicator
                if feed.is_query() {
                    ListItem::new(format!(" 🔍 {}", feed.title()))
                } else {
                    ListItem::new(format!(" {}", feed.title()))
                }
            })
        ...
        if feed.entries().is_empty() {
            ...
            feed.entries()
                ...
                .map(|entry| {
                    ...
                    // For query feeds, show the source feed name
                    if is_query {
                        if let Some(feed_title) = &entry.feed_title {
                            ListItem::new(format!(" {} [{}] {}", date, feed_title, entry.title))
                        } else {
